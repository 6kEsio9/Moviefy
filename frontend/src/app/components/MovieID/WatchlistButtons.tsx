...
export default function WatchlistButtons(){
  ...
  const handleWatchlistChange = (state: string) => {
    ...
    const updatedWatchlist = {watched: updatedWatched!, isWatching: updatedWatching!, willWatch: updatedPlan!}
  ...
  useEffect(() => {
    if(user?.watchList.watched.includes(+movieId!))setWatchStatus("watched")
      else if(user?.watchList.willWatch.includes(+movieId!))setWatchStatus("plan")
        else if(user?.watchList.willWatch.includes(+movieId!))setWatchStatus("watching");
          else setWatchStatus('none')
  }, [])

  return(
  <div style={{position: "absolute", right: "20%"}}>
    <IconButton onClick={() => handleWatchlistChange("watched")} color={watchStatus === "watched" ? "success" : "default"}>
      <RemoveRedEyeRounded/>
      <Typography>{watchStatus === "watched" ? "Watched" : "Watch"}</Typography>
    </IconButton>
    <IconButton onClick={() => handleWatchlistChange("watching")} color={watchStatus === "watching" ? "info" : "default"}>
      <List/>
      <Typography>{watchStatus === "watching" ? "Currently watching" : "Watching"}</Typography>
    </IconButton>
    <IconButton onClick={() => handleWatchlistChange("plan")} color={watchStatus === "plan" ? "secondary" : "default"}>
      <AccessTime/>
